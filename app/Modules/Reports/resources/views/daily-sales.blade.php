        ...
        <div class="col-md-3">
            <div class="card bg-primary text-white">
                <div class="card-body">
                    ...
                    <h2 class="card-text">${{ $report['formatted_metrics']['total_revenue'] ?? '0.00' }}</h2>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">30-Day Sales Summary</h5>
                    <div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="salesDataTable" class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>Date</th>
                            ...
                            <tbody>
                                @foreach($monthlyTrends as $trend)
                                    <tr>
                                        ...
                                        <td class="text-end" data-order="{{ $trend['revenue'] ?? 0 }}"><strong>${{ number_format($trend['revenue'] ?? 0, 2) }}</strong></td>
                                        ...
                                        <td data-order="{{ $trend['revenue'] ?? 0 }}">
                                            @if(($trend['transactions'] ?? 0) == 0)
                                                <span class="badge bg-secondary">No Sales</span>
                                            @elseif(($trend['revenue'] ?? 0) >= 10000)
                                                <span class="badge bg-success">Excellent</span>
                                            @elseif(($trend['revenue'] ?? 0) >= 5000)
                                                <span class="badge bg-primary">Good</span>
                                            @elseif(($trend['revenue'] ?? 0) >= 1000)
...
function exportToExcel() {
    ...
    // Headers
    const headers = ['Date', 'Subtotal', 'Discount', 'Tax', 'Total Revenue', 'Orders', 'Avg Order Value', 'Performance'];
    ...
    link.setAttribute('download', 'daily_sales_30_days.csv');
